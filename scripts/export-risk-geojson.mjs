...
import {
  ...
  inWorkBbox,
  isInIdfWorkArea,
} from './lib/idf-zone.mjs'

const MOTO_CATV = new Set([
  '2', '3', '4', '6', '30', '31', '32', '33', '34', '42', '43',
])
...
const YEARS = YEARS_ARG
  ? YEARS_ARG.replace('--years=', '').split(',').map((y) => y.trim())
...
function loadRoadSegments() {
  ...
  if (fromTiles?.length) {
    console.log(`Réseau routier : ${fromTiles.length} tronçons (tuiles Overpass)`)
    return fromTiles
  }
  return null
}
...
console.log(`Total : ${allPoints.length} accidents moto IDF (${YEARS.join(', ')})`)
